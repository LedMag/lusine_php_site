...
$varSub_articulophone = "0";
if (isset($_GET ["recordID"])) {
  $varSub_articulophone = $_GET ["recordID"];
}
mysql_select_db($database_gabrielle, $gabrielle);
...
$articulophone = mysql_query($query_articulophone, $gabrielle) or die(mysql_error());
$row_articulophone = mysql_fetch_assoc($articulophone);
$totalRows_articulophone = mysql_num_rows($articulophone);

$queryString_articulophone = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_articuloarticulophone") == false && 
        stristr($param, "totalRows_articuloarticulophone") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_articuloarticulophone = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_articuloarticulophone = sprintf("&totalRows_articuloarticulophone=%d%s", $totalRows_articuloarticulophone, $queryString_articuloarticulophone);
...
<!--************************************************************************************************-->
  ...
  <table width="100%" border="0" align="center" class="gotico">
    ...
    <tr>
      ...
      <td width="73%" align="left"><?php echo $row_articulo['strNombre']; ?></td>
      <td width="2%">&nbsp;</td>
      </tr>
    ...
    <tr>
      ...
      <td align="left" valign="top" class="descripcione"><?php echo $row_articulo['strDescripcion']; ?></td>
      <td>&nbsp;</td>
      </tr>
    <?php } // Show if recordset not empty ?>
...
<!--************************************************************************************************-->
  ...
  <table width="100%" border="0" align="center" class="gotico">
    ...
    <?php if ($row_articulophone['strNombre'] > NULL) { // Show if recordset not empty ?>
    <tr>
      <td width="25%" align="right" valign="top"><strong>NOMBRE</strong>:</td>
      <td width="73%" align="left"><?php echo $row_articulo['strNombre']; ?></td>
      <td width="2%">&nbsp;</td>
    ...
    <?php if ($row_articulophone['strDescripcion'] > NULL) { // Show if recordset not empty ?> 
    <tr>
      <td align="right" valign="top" class="descripcione"><strong>DESCRIPCIÓN</strong>:</td>
...
<!--************************************************************************************************--><!--************************************************************************************************--> 
<?php } while ($row_articulophone = mysql_fetch_assoc($row_articulophone)); ?>
<!--************************************************************************************************-->
<!--************************************************************************************************-->
<?php include("../menues/iconos_navegacion_articulo.php"); ?>
<!--************************************************************************************************-->
...
<?php if ($pageNum_articulo > 0) { // Show if not first page ?>
<a href="<?php printf("%s?pageNum_articulo=%d%s", $currentPage, max(0, $pageNum_articulophone - 1), $queryString_articulophone); ?>" title="atrás" onmouseover="MM_swapImage('botonatras','','../gif/controles/atras.png',1)" onmouseout="MM_swapImgRestore()"><img src="../gif/controles/atras1.png" width="40%" id="botonatras" /></a>
<?php } // Show if not first page ?></td>
    <td width="40%">&nbsp;</td>
    <td width="30%" align="right" valign="top">
      <?php if ($pageNum_articulophone < $totalPages_articulophone) { // Show if not last page ?>
    <a href="<?php printf("%s?pageNum_articulophone=%d%s", $currentPage, min($totalPages_articulophone, $pageNum_articulophone + 1), $queryString_articulophone); ?>" title="siguiente" onmouseover="MM_swapImage('sigue','','../gif/controles/siguiente.png',1)" onmouseout="MM_swapImgRestore()"><img src="../gif/controles/siguiente1.png" width="40%" id="sigue" /></a>
  <?php } // Show if not last page ?>
    </td>
  </tr>
</table>
...
mysql_free_result($articulophone);
?>
